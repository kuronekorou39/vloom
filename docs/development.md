...
**送信側の条件を毎回そろえること。** 特に落ちやすいのが 2 つ:

- **`--bpc 1` を明示する**。輝度 4 値は同じ格子の 1bit より遅く、13x18 では
  回収 0/234 になる (experiments.md 2026-08-28)。デスクトップ送信アプリの既定は
  1bit にしてあるが、UI で切り替えたまま気付かないことがある
- **構図は `--geometry` / `--zoom` / `--dx` を控えておく**。送信ウィンドウを開き直すと
  既定の位置・大きさに戻り、三脚の狙いと合わなくなる。送信中の下部バーに現在値が
  `配置 --zoom 0.92 --dx +0.130 --dy +0.000 (2.32px/セル)` の形で出るので、
  合わせ込んだらこの文字列をそのまま記録して次回に渡す。
  受信画面の構図オーバーレイ (充填率・余白・px/セル) と突き合わせると速い
