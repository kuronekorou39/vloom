...
実機実測は **70〜110 KB/s** (縦長 11×14 / 1bit / 20fps / 受信 1600×1200、BenQ 平面
モニタを三脚から約 50cm で撮影、Pixel 9a)。受信はスマホ縦持ちで視野が 3:4 なので、
格子も縦長にして視野を使い切っています。正方形に近い 11×10 だと約 60 KB/s、9×8 で
約 40、7×6 で約 22、5×4 で約 10 KB/s です。端末が温まるとスキャン速度が落ちて 2 倍
以上変わるので、その日の最初の 1 回を代表値にしないこと。詳細は
[docs/vcode_format.md](docs/vcode_format.md) と
