...
実機実測は **1MB の転送で 131〜147 KB/s (平均 138)**
(縦長 13×18 / 1bit / 20fps / 受信 1600×1200 / 露出補正 −1 EV、BenQ 平面モニタを三脚から
約 50cm で撮影、Pixel 9a)。受信はスマホ縦持ちで視野が 3:4 なので、格子も縦長にして
視野を使い切っています。11×14 で約 65、11×10 で約 60、9×8 で約 40、7×6 で約 22、
5×4 で約 10 KB/s です。短いファイルの値は立ち上がりの瞬間風速で、定常状態は 1MB 以上で
測ること。端末が温まるとスキャン速度が 2〜3 倍遅くなるので、温度も揃えること。詳細は
[docs/vcode_format.md](docs/vcode_format.md) と
[docs/measurements/](docs/measurements/)。

受信は露出を −1 EV 下げています。露光が短いほど、表示の切り替わりがローリング
シャッターの 1 枚に混ざる帯が狭くなり、ばらつきが縮みます。下げすぎると構図によって
白が沈んでヘッダが読めなくなるので、控えめにしてあります。

**構図合わせ**: 受信画面はコードを掴むと、カメラ画像に対する充填率・四辺の余白・
px/セル を読み上げます (例: `充填 幅 72% · 高 68%  4.6 px/セル / 余白 上 21% 下 3% ...`)。
余白が偏っていればその方向に寄せ、どこかが 2% を切ると赤で「はみ出し注意」が出ます。
緑のガイド枠は選択中の格子の縦横比になります。
...
`+` / `-` で拡縮、`0` で戻ります (Shift で粗く動きます)。`H` (または `--hold`) で
フレームを止めて 1 枚を出し続ける「静止」になり、構図を合わせている間に受信が
完走して画面が変わったり、切り替わりでちらついたりしなくなります。カメラを三脚に据えている
