...
実機実測は **6.5〜16.9 KB/s** (7×6 / 1bit / 10fps / 1080p 受信、BenQ 平面モニタを三脚から
約 50cm で撮影)。幅があるのは端末の熱状態によるもので、冷えていれば理論値 17.2 KB/s の
98%、温まると 38% まで落ちます。6fps に下げると温間でも理論値をほぼ取り切ります
(10.5 KB/s)。詳細は [docs/vcode_format.md](docs/vcode_format.md)。
...
理論との差を埋めきれない主因はローリングシャッターで、センサーが 1 枚を読み出す
25〜35ms の間に送信側の画面が切り替わると、1 枚の写真に 2 フレームが混ざって
データブロックが全滅します。このため fps を上げても稼げません。輝度 4 値 (2bit) は
理論こそ 2.2 倍ですが、LCD の残像と合わさって中間 2 レベルが読めず実機では成立
しませんでした (100KB が 20 分でも完了せず、1bit なら 15.8 秒)。容量はセル数
(格子) で稼ぐ方針です。

[docs/vcode_format.md](docs/vcode_format.md) の実測表は 2026-07-02 時点のもので、
2bit の値はその後の実機検証 (2026-08-04) で覆っています。
