...
PKG = "app.vloom.vloom.lab"
ACTIVITY = f"{PKG}/app.vloom.vloom.MainActivity"
...
def one_run(extras: list[str], timeout: float) -> dict[str, str] | None:
    """1 回ぶん起動して、完了の統計行を待つ。時間切れなら None。"""
    adb("logcat", "-c")
    adb("shell", "am", "force-stop", PKG)
    time.sleep(1.0)
    adb("shell", "am", "start", "-n", ACTIVITY, "--ei", "tab", "1", *extras)
...
def main() -> int:
    ...
    for i in range(args.runs):
        ...
        for name, extras in arms:
            ...
            stats = one_run(extras, args.timeout)
            if stats is None:
                print("時間切れ")
                continue
            k = kbps_of(stats)
