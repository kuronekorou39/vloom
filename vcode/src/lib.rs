...
impl Layout {
    ...
    /// 縦長: 220x294 セル、154 ブロック。幅は V3_MAX と同じまま高さだけ伸ばす。
    ///
    /// 受信はスマホ縦持ちで、カメラ視野は 3:4 (1200x1600) になる。正方形に近い
    /// 格子だと幅に合わせた時点で縦が 700px ほど余る。幅を変えなければ px/セル
    /// (= 読み取りマージン) は変わらないので、余っている縦を使い切るだけで
    /// ブロック数を 1.4 倍にできる。送信面が横長モニタだと画面の左右が余るが、
    /// スループットには効かない (どのみち高さが上限になるため)。
    pub const V4_TALL: Layout = Layout { block: Self::BLOCK, grid_w: 11, grid_h: 14 };

    /// 超密: 260x254 セル、156 ブロック。V4_TALL とブロック数はほぼ同じだが、
    /// 幅を広げるぶん px/セル が落ちる (ガイド枠 0.8 x 1200px で 3.7px/セル)。
